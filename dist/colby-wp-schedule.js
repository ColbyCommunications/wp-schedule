...
var EventPicker = function () {
  ...
  _createClass(EventPicker, [{
    ...
    value: function handleCheckBoxChange(event) {
      if (event.target.checked === true && this.activeTags.indexOf(event.target.value) === -1) {
        this.activeTags.push(event.target.value);
      } else if (event.target.checked === false && this.activeTags.indexOf(event.target.value) !== -1) {
