...
class SchedulePickerShortcode {
	...
	public static function schedule_picker( $atts = [] ) : string {
		...
		$parent_term = get_term_by( 'name', $atts['schedule'], 'schedule_category' );
		if ( empty( $parent_term ) ) {
			return '';
		}

		$terms = get_term_children( $parent_term->term_id, 'schedule_category' );
		if ( empty( $terms ) ) {
			return '';
		}
		...
		$extra = '
			<h4 class="mt-4">
				<a href="' . get_term_link( $parent_term->term_id ) . '">View all events</a>
			</h4>';
		$output = '
		...
		$output .= array_reduce(
			...
			function( $output, $term ) {
				$term = get_term( $term, 'schedule_category' );
